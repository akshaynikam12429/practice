...
void thread_function(int sock_fd) {
    ...
    char *payload = malloc(strlen(check.payload)+1);
    strcpy(payload,check.payload);
    ...
    if(!strcmp(transform_value,"CSV")){
        ...
        printf("payload dasta is %s\n",payload);
        payload_data_file = transformCSV(payload);         
        printf("\n\nfilp is : %s\n\n\n",payload_data_file);
    ...
    if(!strcmp(transport_value,"email")){
        printf("file to be read in email function is %s\n\n\n",payload_data_file);
